...
import {
  ...
  Building2, FileText, UserCog, Plus, Layers
} from 'lucide-angular';
...
export class EditHospitalarioComponent extends BaseCatalogEditComponent implements OnInit {
  protected readonly icons = {
    X, Save, Loader2, Search, Trash2, Check, Package, Building2, FileText,
    UserCog, Plus, Layers
  } as const;
  ...
  ngOnInit(): void {
    this.loadInsumos();
    this.loadMedicos();
    this.loadCatalogForSugerencias('HOSPITALARIO');
  }
  ...
  public save(): void {
    ...
    if (this.isEditing() && this.itemId()) {
      this.catalogService.updateItem(this.itemId()!, itemData as CatalogItem).subscribe({
        ...
        error: () => { this.isSaving.set(false); console.error('Error updating hospitalario item'); }
      });
    } else {
      this.catalogService.createItem(itemData).subscribe({
        next: (newId: string) => this.saveRecipeAndFinish(newId),
        error: () => { this.isSaving.set(false); console.error('Error creating hospitalario item'); }
      });
    }
  }
