...
import {
  ...
  Pill, FileText, UserCog, Plus, Layers
} from 'lucide-angular';
...
export class EditMedicamentoComponent extends BaseCatalogEditComponent implements OnInit {
  protected readonly icons = {
    X, Save, Loader2, Search, Trash2, Check, Package, Pill, FileText,
    UserCog, Plus, Layers
  } as const;
  ...
  public save(): void {
    ...
    if (this.isEditing() && this.itemId()) {
      this.catalogService.updateItem(this.itemId()!, itemData as CatalogItem).subscribe({
        ...
        error: () => { this.isSaving.set(false); console.error('Error updating medicamento item'); }
      });
    } else {
      this.catalogService.createItem(itemData).subscribe({
        next: (newId: string) => this.saveRecipeAndFinish(newId),
        error: () => { this.isSaving.set(false); console.error('Error creating medicamento item'); }
      });
    }
  }
