...
export class FacturacionComponent {
  ...
  async cargarServicioAsync(servId: string, skipSuggestions = false): Promise<void> {
    ...
    if (pId === null) {
      ...
      if (!yaEnCarrito) {
        this.carritoLocal.update(prev => [...prev, new CatalogItem({
          ...
          medicoId: (esConsulta || (s.honorarioBase ?? 0) > 0) ? this.selectedMedicoId() : undefined,
          medicoNombre: (esConsulta || (s.honorarioBase ?? 0) > 0) ? this.nombreMedicoSeleccionado() : undefined,
    ...
    const payload: CargarServicioACuentaRequest = {
      ...
      medicoId: (esConsulta || (s.honorarioBase ?? 0) > 0) ? this.selectedMedicoId() || undefined : undefined,
    ...
    try {
      ...
      this.serviciosEnBackend.update((prev: any[]) => [...prev, new CatalogItem({
        ...
        medicoId: (esConsulta || (s.honorarioBase ?? 0) > 0) ? this.selectedMedicoId() : undefined, // Guardar ID para limpieza de cita
        hora: this.horaCita(),
        medicoNombre: (esConsulta || (s.honorarioBase ?? 0) > 0) ? this.nombreMedicoSeleccionado() : undefined,
  ...
  imprimirRecibo(reciboId: string) {
    this.facturacionService.getReceiptPrintData(reciboId).subscribe({
      next: (data: any) => {
        ...
        this.printService.print(content, `Recibo ${printData.numeroRecibo}`);
      },
