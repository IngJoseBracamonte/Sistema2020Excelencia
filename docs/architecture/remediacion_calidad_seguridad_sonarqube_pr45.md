...
| :--- | :--- | :--- | :--- | :--- |
| `src/SistemaSatHospitalario.Frontend/src/app/features/admin/catalog/catalog-management.component.html:25` | `Web:MouseEventWithoutKeyboardEquivalentCheck` | **BUG (Minor)** | `angular-frontend-specialist` | Se incorporó `(keydown.escape)="closeCreateDropdown()"`, `tabindex="0"`, `role="button"` y `aria-label="Cerrar opciones de creación"` en el backdrop invisible de cierre. |
...
| `playwright-runner.js:10` | `javascript:S4036` | **VULNERABILITY (Minor)** | `appsec` | Se aisló el entorno de ejecución de subprocesos definiendo un `PATH` inmutable y no modificable (`/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin`) pasado de forma explícita al contexto de opciones de `exec`. |
