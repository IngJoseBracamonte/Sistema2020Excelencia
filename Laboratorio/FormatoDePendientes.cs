...
{
    public partial class FormatoDePendientes : Form
    {
        ...
        private void HojadeImpresion(PdfSharp.Pdf.PdfDocument document)
        {
            try 
            {
                ...
                XRect Margen = new XRect(180, 15, 145, 14);
                string Titulo = "COMPROBANTE DE PAGO PENDIENTE";
                ...
                Margen = new XRect(10, PosicionP, 145, 14);
                cmd = "Fecha: ";
                cmd2 = Convert.ToDateTime(Persona.Tables[0].Rows[0]["fecha"].ToString()).ToString("dd/MM/yyyy");
                ...
                PosicionP = PosicionP + 15;
                DataSet Deuda = new DataSet();
                Deuda = Conexion.Deuda(IdOrden.ToString());
                Margen = new XRect(10, PosicionP, 145, 14);
                Double BolivaresT = Convert.ToDouble(Persona.Tables[0].Rows[0]["PrecioF"].ToString());
                if (Deuda.Tables[0].Rows.Count > 0)
                {
                    BolivaresT = Convert.ToDouble(Deuda.Tables[0].Rows[0]["Total"].ToString());
                }
               
                Double BolivaresF = Convert.ToDouble(Tasa.Tables[0].Rows[0]["Dolar"].ToString());
                string TotalF = DateTime.Now.ToString("dd/MM/yyyy");
        ...
        private void FormatoDePendientes_Load(object sender, EventArgs e)
        {
            IdOrden = CobroPendientes.IdOrden;
            ...
            try
            {
                ...
                pdfViewer2.Renderer.Load(document1);
                pdfViewer2.Renderer.Zoom = 2.2;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString());
        ...
        private void FormatoDePendientes_FormClosing(object sender, FormClosingEventArgs e)
        {
            document.Dispose();
            document1.Dispose();
            try
