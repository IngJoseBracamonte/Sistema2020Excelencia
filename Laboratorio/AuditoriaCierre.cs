...
{
    public partial class AuditoriaCierre : Form
    {
        public AuditoriaCierre()
        {
            InitializeComponent();
        }

        private void AuditoriaCierre_Load(object sender, EventArgs e)
        {
        ...
        private void iconButton3_Click(object sender, EventArgs e)
        {
            ...
            ds = Conexion.SELECTTotalFacturadoFecha(cmd, cmd2);
            if (ds.Tables[0].Rows.Count != 0)
            {
                if (ds.Tables[0].Rows[0]["SumaDePrecioF"].ToString() != " " && ds.Tables[0].Rows[0]["SumaDePrecioF"].ToString() != "")
                {
                    Nombre.Text = ds.Tables[0].Rows[0]["SumaDePrecioF"].ToString();
                }
                else
                {
                    Nombre.Text = "0";
                ...
                dsT = Conexion.SELECTTotalFacturadoDia(cmd, cmd2);
                if (dsT.Tables[0].Rows[0]["Total"].ToString() != " " && dsT.Tables[0].Rows[0]["Total"].ToString() != "")
                {
                    textBox1.Text = dsT.Tables[0].Rows[0]["Total"].ToString();
                }
                else
                {
                    textBox1.Text = "0";
                ...
                string Text1, Text2;
                Text1 = Nombre.Text.Replace(".", ",");
                Text2 = textBox1.Text.Replace(".", ",");
                PrecioF = Convert.ToDouble(Text1);
                Total = Convert.ToDouble(Text2);
                Calculo = Total - PrecioF;
                textBox2.Text = Calculo.ToString("N2", CultureInfo.CreateSpecificCulture("es-VE"));
            }
            else
            {
                Nombre.Text = "0";
            ...
            ds2 = Conexion.TotalDetalladoPorFecha(cmd,cmd2);
        ...
        private void iconButton2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                i++;
                sl.SetCellValue(i, 2, Nombre.Text);
                sl.SetCellValue(i, 3, textBox1.Text);
                sl.SetCellValue(i, 4, textBox2.Text);
                i++;
