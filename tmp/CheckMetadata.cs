...
namespace CheckMetadata
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            // 3. Buscar algun paciente con Legacy ID para usarlo
            if (p1 == null || p1.IdPacienteLegacy == 0) {
                var candidate = await context.Pacientes.Where(p => p.IdPacienteLegacy > 0).Take(1).FirstOrDefaultAsync();
                if (candidate != null) Console.WriteLine($"Sugerencia: Usa a '{candidate.NombreCompleto}' (Legacy ID: {candidate.IdPacienteLegacy})");
            }
        }
    }
}
